...
export const isRoleButton = (customId: string | undefined): boolean => typeof customId === 'string' && customId.startsWith('role:')
...
export async function handleRoleButton(json: APIMessageComponentInteraction, botToken: string) {
	...
	const statusText = ok ? (hasRole ? `Снята роль \`${kind}\`.` : `Выдана роль \`${kind}\`.`) : 'Не удалось изменить роль. Проверьте права бота и иерархию ролей.'
